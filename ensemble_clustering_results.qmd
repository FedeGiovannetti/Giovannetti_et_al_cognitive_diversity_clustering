...
mean_sd <- function(variable){
  ...
  (Table1 <- datos.reducidos %>% 
    left_join(subject_info, by = "username") %>% 
    summarise(n = as.character(n()),
              girls = as.character(sum(Sex == "fem", na.rm = TRUE)),
              Age = mean_sd(Age),
              'Parent education level' = mean_sd(EducacionNES),
              'Parent ocupation level' = mean_sd(OcupacionNES),
              Housing = mean_sd(ViviendaNES),
              Overcrowding = mean_sd(HacinamientoNES),
              'SES Score' = mean_sd(NES),
              'UBN' = paste(
                round(mean(UBN, na.rm = T)*100, digits = 2),
                "%")
              ) %>% 
    pivot_longer(everything()) %>%
    flextable() %>% 
    # custom_flextable("")%>%
    set_header_labels(name = "Table 1. Sample information", value = "") %>%  # Remove column names
    ...
    footnote(
      ...
      j = 1,
      ref_symbols = "a",
      value = as_paragraph("Highest educational and occupational levels reached by parents (range 0 to 12 points)")
      ) %>% 
    footnote(
      i = 6, 
      j = 1,
      ref_symbols = "b",
      value = as_paragraph("Scale range: 3–12 points, with higher scores for better housing conditions")
      )%>% 
    footnote(
      i = 7, 
      j = 1,
      ref_symbols = "c",
      value = as_paragraph("Scale range: 0–9 points, with higher scores for better conditions.")
      ))
...
## 2.1.Table 2. Descriptive and inferential analyses for clustering result

```{r clustering descriptive table}
...
## 2.2.Table 3. Pairwise comparisons for clustering result.

```{r cognitive pairwise comparisons}
...
save_table(Table3, "Tables/Table_3.docx")
...
```{r cognitive comparison plot, fig.width= 8, fig.height=6}
  ...
  df %>% 
    ggplot(aes(x = Feature, 
      ...
      scale_color_discrete(
        name = "Clusters",
        labels = c(
          "Cluster 1" = "**Cluster 1**<br>(Working memory<br>low-performance group)",
          "Cluster 2" = "**Cluster 2**<br>(Generalized<br>high-performance group)",
          "Cluster 3" = "**Cluster 3**<br>(Generalized<br>low-performance group)"
        ),
        guide = guide_legend(
      ...
      theme_minimal(base_size = 15) +
        ...
        theme(
          ...
          legend.text = element_markdown(lineheight = 1.2) # markdown rendering
        )
}
...
## 3.2. Table S4. Comparison of temperamental and contextual variables between clusters (Kruskal Wallis's test)

```{r subject info between clusters}
